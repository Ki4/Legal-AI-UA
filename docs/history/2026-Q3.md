...
Sections arrive here from the ROADMAP when they age past the three most recent, and they arrive
...
**Audited on the way out (2026-09-01b).** Every lesson above is carried by something that runs:
the checker exclusion by `pnpm check:copy`, which now scans every file and has its own tests; the
`BlockTrust`-versus-`confirmed` distinction and the `selected_by` / `tool_calls` rendering by the
anatomy probes, which turn a named test red when either is undone; `started_at`'s absence from the
view model by the probe that tries to let it ride along on a spread. Nothing in this section was
carried by memory alone, so nothing from it became a debt.
...
**Audited on the way out (2026-09-02).** The block tree's scope is carried by ADR-0013 and
`specs/admin-console.md` — a decision about what the MVP is, which no gate can hold. Both freeze
lessons — a version guarded while its blocks stay writable, and `version_is_frozen` reading "not
frozen" for a row it cannot see — are carried by scenarios in `verify_document_blocks.sql`, which
`pnpm verify:sql` runs in CI. The fourth was carried by nothing until the day it was archived: "run
the ledger repair, the line is already there" was safe under its premise and would have buried five
migrations under the facts, and only on 2026-09-02 did `pnpm check:cloud-ledger` start asking the
cloud instead of trusting the instruction.
...
**Audited on the way out (2026-09-02).** The shell-versus-`/print` split is carried by `read.ts`
fetching both, by `rada.test.ts`'s fixtures and by ADR-0023 — and, since 2026-09-02, by a live
request that read an article end to end. "A parser is named for its source" is ADR-0023 itself.
The fixture-whitespace lesson is carried by the `parser-welds-an-article-into-one-line` probe, and
`check:sql` holding the `audit_change` map by `audit-mapping-loss-goes-unnoticed`. Two are carried
less than fully and both are recorded as such rather than waved through: the cheap-probe split is
carried by spec §9.7 as an intent whose gate arrives with ADM-44, and **`text_blank` measured over
the right slice is carried by one test and no probe** — the lesson was that an assertion over the
wrong slice cannot fail, and nothing watches that this one still measures the right thing. That gap
is a debt in STATE, dated the day this section was archived.
...
**Archived 2026-09-17, audited.** Where each lesson lives now: the mount and the copy that
cannot go stale — ADR-0025, `scripts/sync-edge-shared.mjs` with its test and the probe
`edge-shared-copy-goes-stale`, and the rule in `supabase/CLAUDE.md` to serve through
`pnpm functions:serve`; a package joining the Deno copy on the day it is imported — a rule only, and
a debt in STATE since 2026-09-02; RLS bypass versus privileges, and a comment that became a grant —
the rule in `supabase/CLAUDE.md` and the `service_role` scenarios in `verify_law_revisions.sql` and
`verify_grants.sql`; a prediction not being a mechanism — the quality-gates principle in the root
`CLAUDE.md` and `pnpm probes`, with the concrete gap (nothing runs an edge function in CI) a debt in
STATE since 2026-09-02. Nothing in this section is carried by nothing.
...
**Archived 2026-09-18, audited.** Where each lesson lives now: a cadence counting attempts —
`verify_law_due.sql`, whose scenarios sort on `last_checked_at` and would fail on
`last_verified_at`; the load-bearing exclusion — scenario 3 of the same file, "an act-scoped norm is
excluded — null sorts first"; a decision shared by import — `law-sweep/sweep.ts` imports `checkNorm`
from `_shared/law-refs`, and the probe `edge-shared-copy-goes-stale` holds the copy it reaches it
through, though no rule says "do not rewrite §9.7" in words; a gate that prints the fix —
`sql.yml` itself, whose types step prints the regeneration diff; two halves deferred out loud — the
debts in STATE dated 2026-09-01 (no edge-function secret) and 2026-09-07 (the cheap tier). Nothing
in this section is carried by nothing.
...
**Archived 2026-09-19, audited.** Where each lesson lives now: a hook rewrites the claim —
`apps/console/src/app/claims.ts` and `claims.test.ts`, which read the token and nothing else; a
config file has a scope, and `db reset` restarts the database and not the auth container — both
were carried by nothing but ADR-0026 and a journal entry, and are now a rule in
`supabase/CLAUDE.md` ("A role is a `user_roles` row"); a gate red for the wrong reason —
`sql.yml`'s `if: false` with its dated comment, the debt in STATE dated 2026-09-17, and the
"Parked since" paragraph in `supabase/CLAUDE.md`; the gate that caught its author — `check:sql`
rule 4 and the test beside it. Nothing in this section is carried by nothing.
