...
export const PROBES = [
  ...
  // A missing ledger row has two causes needing opposite responses, and the
  // row alone cannot say which. This probe puts the old prescription back —
  // the one that would have sent 2026-09-02 into a `db push` against a schema
  // that already existed.
  {
    id: "missing-ledger-row-prescribes-db-push",
    what: "tells the reader to `db push` a migration whose objects may already be in the cloud",
    file: "scripts/check-cloud-ledger.mjs",
    test: "scripts/check-cloud-ledger.test.mjs",
    from: `          "The ledger cannot say which of two this is: a migration that never ran anywhere, or one " +`,
